...
function _ppApiUrl(action, extra = {}) {
    const endpoint = (window.ENV.OMISE_API_ENDPOINT || '').replace(/\/$/, '');
    const base     = (window.ENV.PROMPTPAY_API_BASE  || '').replace(/\/$/, '');
    const isLocal  = ['localhost', '127.0.0.1'].includes(window.location.hostname);
    // localhost → PHP เสมอ ไม่สนใจ OMISE_API_ENDPOINT (cloud)
    const url = (!isLocal && endpoint)
        ? endpoint
        : (base ? `${base}/api/omise_payment.php` : 'api/omise_payment.php');
    const qs  = new URLSearchParams({ action, ...extra });
    return `${url}?${qs}`;
}

async function startPromptPayPayment() {
    const endpoint = window.ENV.OMISE_API_ENDPOINT  || '';
    const apiBase  = window.ENV.PROMPTPAY_API_BASE  || '';
    const isLocal  = ['localhost', '127.0.0.1'].includes(window.location.hostname);
    if (!isLocal && !endpoint) {
        showPosAlert('PromptPay: ตั้งค่า OMISE_API_ENDPOINT ใน env.js ให้ชี้ไป Supabase Edge Function');
        return;
    }
    ...
    try {
        ...
        if (err instanceof TypeError && err.message.toLowerCase().includes('fetch')) {
            const isLocal = ['localhost', '127.0.0.1'].includes(window.location.hostname);
            showPosAlert(isLocal
                ? 'เชื่อมต่อไม่ได้ — เปิด XAMPP Apache และใช้ http://localhost/ ไม่ใช่ file://'
                : 'ระบบ PromptPay ยังไม่พร้อม — Supabase Edge Function ยังไม่ได้ deploy (ดู env.js)'
            );
        } else {
