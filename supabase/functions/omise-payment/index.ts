// Supabase Edge Function — Omise PromptPay API proxy
...
const OMISE_CONFIGURED =
  OMISE_SECRET_KEY.startsWith('skey_test_67oozc6tp14390get0c') &&
  !OMISE_SECRET_KEY.includes('xxxxxxxx');
...
async function createCharge(req: Request): Promise<Response> {
  ...
  if (amountSatang < 100) return json({ error: `ยอดเงินต้องไม่น้อยกว่า 1 บาท (ได้รับ: ${amountBaht})` }, 400);
  ...
  if (res.status !== 200 || !data.id) {
    ...
    console.error(`[Omise] charge failed [${res.status}]:`, data);
...
// ── Expire (cancel) charge immediately ──────────────────────
async function expireCharge(req: Request): Promise<Response> {
  ...
  if (res.status !== 200) {
    return json({ error: `Omise: ${data.message ?? data.code}`, http_code: res.status }, 502);
  }
